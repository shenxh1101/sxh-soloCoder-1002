...
export function MaterialList() {
  ...
  const topicMaterials = materials.filter((m) => m.topicId === activeTopicId);
  const confirmedCount = topicMaterials.filter((m) => m.confirmed).length;
  const totalCount = topicMaterials.length;
  ...
  return (
    <Card className="h-full flex flex-col overflow-hidden animate-fade-in" style={{ animationDelay: '300ms' }}>
      <CardContent className="flex-1 overflow-y-auto min-h-0 p-4">
        <TopicHeader moduleIcon="📚" moduleName="素材清单" />
