...
export function Timeline() {
  ...
  const getTimelineTrack = () => {
    ...
    return (
      <div className="relative h-8 bg-slate-900/50 rounded-lg overflow-hidden mb-4">
        <div className="absolute inset-0 flex">
          {filteredItems.map((item, index) => {
            ...
            return (
              <div
                key={item.id}
                className="h-full flex items-center justify-center group relative transition-all duration-300 hover:brightness-110"
                style={{
                  width: `${widthPercent}%`,
                  backgroundColor: typeConfig?.color ? `var(--tw-color)` : undefined,
                }}
              >
                <div
                  className={`absolute inset-0 ${typeConfig?.color} opacity-80`}
                />
                <span className="relative z-10 text-[10px] text-white font-medium truncate px-1">
                  {item.title}
                </span>
                {item.marker && (
      ...
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 text-sm text-slate-400">
            <Clock size={14} />
            <span>总时长: </span>
            <span className="font-mono text-amber-400">{formatTime(totalDuration)}</span>
          </div>
        ...
        {filteredItems.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredItems.map((item) => (
              <TimelineItem key={item.id} item={item} totalDuration={totalDuration} />
            ))}
          </div>
        )}
      </CardContent>
